...
```mermaid
flowchart LR
    subgraph FD ["Устройство Извличане/Декодиране (7 степени)"]
        IFU["IFU\n32B/такт от L1 кеш"] --> DEC["3 декодера\n4-1-1 µops/такт"]
        DEC --> RAT["RAT\n40 физически регистъра"]
        IFU -.-> BTB["BTB\n512 записа\nпредсказване на преходи"]
    end
    subgraph DE ["Диспечер/Изпълнение"]
        RS["RS Reservation Station\nизвън ред стартиране"] --> EXEC["5 порта:\n2×IU | 2×FPU | 1×LS"]
    end
    subgraph RET ["Завършващо устройство"]
        ROB_OUT["3 µops/такт\nпо реда в програмата\nпрекъсвания/изключения"]
    end
    ...
    EXEC -->|резултати| ROB
    ROB --> RET
    RET --> ARCH["Архитектурни регистри\n/ Памет"]
    FD <--> BIU["Шинен интерфейс\nL2 кеш (6×8b)"]
    DE <--> BIU
```
